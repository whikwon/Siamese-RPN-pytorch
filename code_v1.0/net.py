...
class SiameseRPN(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(                  #1, 3, 256, 256
            nn.Conv2d(1, 64, kernel_size=11, stride=2), #1, 64,123, 123
    ...
    def forward(self, template, detection):
        ...
        ckernel = self.conv1(template)
        ckernel = ckernel.view(2* self.k, 256, 4, 4)
        cinput  = self.conv3(detection)


        rkernel = self.conv2(template)
        rkernel = rkernel.view(4* self.k, 256, 4, 4)
        rinput  = self.conv4(detection)

        coutput = F.conv2d(cinput, ckernel)
        routput = F.conv2d(rinput, rkernel)
